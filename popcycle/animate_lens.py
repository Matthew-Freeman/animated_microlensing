...
sourceimage = Image.open('orange-popsicle-clipart-ice-cream-bar-ice-pop-mailbox-letterbox-transparent-png-2745707.png')
sourcelocation = [260*s,310*s]   #starting location of the source image (centre)
centred = False         # centres the source image, overwrites sourcelocation (use if source is static background)
source_step = [2.5*s,0*s]    # x,y velocity (pixels per frame)
source_scaling = 0.5*s         #scale the source image by this factor

lenses = [ [350*s,300*s,200*s],	#lens [x, y, einsteinradius]
			] #[480,370,0],
			#]
...
def main():
	...
	for frame in range(n_frames):		#for each frame of the movie
		...
		dot_rad = 10
		# print((lenses[0][0]+lenses[0][2],lenses[0][1]+lenses[0][2]),(lenses[0][0]-lenses[0][2],lenses[0][1]-lenses[0][2]))
