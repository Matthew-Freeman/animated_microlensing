...
if HD:
	s=2
framesize = (960*s,540*s)   #size of output video
n_frames = int(760/10)+1			 
framerate = 30   		#frames per second
filename = 'bagel_demonstration_'

sourceimage = Image.open('roll1.jpg')
sourcelocation = [100*s,250*s]   #starting location of the source image (centre)
centred = False         # centres the source image, overwrites sourcelocation (use if source is static background)
source_step = [10*s,0*s]    # x,y velocity (pixels per frame)
source_scaling = 0.8*s         #scale the source image by this factor

lenses = [ [480*s,270*s,200*s],	#lens [x, y, einsteinradius]
			] #[480,370,0],
			#]
...
max_brightness = 260 #4.2
min_brightness = 220 #0.4
...
def main():
	...
	for frame in range(n_frames):		#for each frame of the movie
		...
		if show_plots or markers:
			...
			brightness = np.mean(255-image_array)
			print(brightness)
			centroid = centroid_com(255-flat_array)
			print(centroid)
		...
		if show_plots:
			...
			draw.text((magplot_location[0]-10*s,magplot_location[1]-magplot_size[1]/2),'Bagelness',fill=(opacity,opacity,opacity,255),font=font,anchor='rm')
